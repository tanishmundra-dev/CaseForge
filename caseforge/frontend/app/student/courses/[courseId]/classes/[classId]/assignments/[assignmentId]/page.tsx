...
function CodingSandbox({ assignment, courseId, classId, assignmentId }: { assignment: AssignmentDetail; courseId: string; classId: string; assignmentId: string }) {
  const [code, setCode] = useState(assignment.starter_code || "# Write your solution here\n");
  ...
  const handleRun = async () => {
    ...
    try { const r = await fetchAPI("/trainee/run", { method: "POST", body: JSON.stringify({ code }) }); setRunOutput({ stdout: r.stdout || "", stderr: r.stderr || "" }); }
    ...
    <div className="theme-dark" style={{ background: "var(--bg-primary)", color: "var(--text-primary)", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", height: "calc(100vh - 44px)", overflow: "hidden" }}>
        ...
        <div style={{ width: "50%", display: "flex", flexDirection: "column" }}>
          <div style={{ background: "var(--bg-tertiary)", borderBottom: "1px solid var(--border)", padding: "8px 16px", fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-secondary)" }}>solution.py</div>
          <div style={{ flex: 1 }}>
            <Editor height="100%" language="python" theme="vs-dark" value={code} onChange={(v) => setCode(v || "")} options={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, minimap: { enabled: false }, padding: { top: 16 }, scrollBeyondLastLine: false, lineNumbers: "on", renderLineHighlight: "gutter" }} />
          </div>
          {runOutput && (
            <div style={{ background: "var(--bg-tertiary)", borderTop: "1px solid var(--border)", padding: "12px 16px", maxHeight: 200, overflowY: "auto" }}>
        ...
        {/* Right — AI Companion */}
        <div style={{ width: "25%", borderLeft: "1px solid var(--border)", display: "flex", flexDirection: "column", padding: "20px 16px 16px", background: "var(--bg-secondary)" }}>
